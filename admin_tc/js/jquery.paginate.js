(function($) {
	...
	$.fn.paginate = function(options) {
		...
		var settings = $.extend({
			...
			controlNext: $('<a href="javascript:void(0);" >Lanjut</a>'),
			controlFirst: $('<a href="javascript:void(0);">Pertama</a>'),
			controlPrev: $('<a href="javascript:void(0);">Sebelum</a>'),
			controlLast: $('<a href="javascript:void(0);" >Terakhir</a>'),
			controlPage: $('<a href="javascript:void(0);" class="page"></a>'),
			controlPagesVisible: 5,
			items: null,
			status: $('#pagination_status'),
			statusTemplate: $("<p style=color:#000>Jumlah Peserta {0} - {1} dari {2}</p>"),
			controlOptions: $('<span class="items_per_page">Jumlah Peserta maksimal: <select name="items_per_page" id="items_per_page"></select></span>'),
			currentPage: 0,
