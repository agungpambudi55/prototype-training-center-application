...
$query_instruktur	=mysql_query("select *from tb_instruktur where status='aktif'");
...
if(is_array(@$idins)){
while($row_instruktur=mysql_fetch_array($query_instruktur))
{
	foreach($idins as $valu)
	{
		if($row_instruktur['id_instruktur'] == $valu){}
		else{
		echo "<input type='radio' name='instruktur' value='".$row_instruktur['id_instruktur']."' required>".$row_instruktur['nama']."<br>";
		}
	}
}
}
