...
<a href="index.php?page=nilai&pages=nilai_insert_tanpa_status"><button class="button_a"  style="padding:8px 10px; margin-right:5px">Tambah Data</button></a>
...
<?php 
if(@$_POST['cari']=="")
{echo "<a href='pages/nilai_print_tanpa_status.php' class='excel' >Laporan Nilai Semua Kelas</a>";
...
{	echo "<a href='pages/nilai_kelas_print_tanpa_status.php?print=$_POST[cari]&kelas=$nama_kelas' class='excel'>Laporan Nilai per Kelas</a>&nbsp;&nbsp;";
	echo "<a href='index.php?page=nilai&pages=nilai_view_tanpa_status' class='button_a'>Lihat Semua</a>";	
}
?>
...
if(@$_POST['cari']=="")
{
$query="SELECT * FROM `tb_nilai` WHERE VAL = '2' ORDER BY `tb_nilai`.`id_judul` ASC LIMIT $posisi,$batas";
...
$pelatihan=$d['pelatihan'];	
	...
	<td align='center'>
	<a href='index.php?page=nilai_update_tanpa_status&update_id=$id' class='update'></a>
    <a href='index.php?page=nilai_delete_tanpa_status&hapus_id=$id'  class='delete' onclick='return kon(\"".$peserta."\")'></a>
	</td>       
  </tr>";	
}
...
<?php
$qry_2 = mysql_query("SELECT * FROM `tb_nilai` WHERE VAL = '2' ORDER BY `tb_nilai`.`id_judul` ASC");
...
if ($halaman > 1)
{$prev = $halaman-1;echo "<a href='index.php?page=nilai&pages=nilai_view_tanpa_status&i=$prev' class='prev'></a>";}
else 
...
if($halaman<$jmlhal)
{$next = $halaman+1; echo "<a href='index.php?page=nilai_view_tanpa_status&i=$next' class='next'></a>";
}
...
tb_peserta_kelas.id_pilih_judul = tb_pilih_judul.id_pilih_judul AND 
tb_kelas.id_kelas = tb_peserta_kelas.id_kelas AND 
tb_kelas.nama_kelas LIKE '%$_POST[cari]%'
and tb_nilai.VAL='2' 
ORDER BY `tb_peserta_kelas`.`no_peserta` ASC");
...
while ($arr_1=mysql_fetch_array($qry_cari))
{
	...
	<td align='center'>
	<a href='index.php?page=nilai_update_tanpa_status&update_id=$arr_1[6]' class='update'></a>
    <a href='index.php?page=nilai_delete_tanpa_status&hapus_id=$arr_1[6]'  class='delete' onclick='return kon(\"".$arr_1['3']."\")'></a>
	</td>       
  </tr>";	
}
}	
